...
export default function App() {
          ...
          setSelectedClientId(prev => {
            if (!prev && result.clients[0]) {
              ...
              if (result.clients[0].data?.findings?.length > 0) {
                ...
                setHistory(result.clients[0].data.history || SEED_HISTORY);
              }
  ...
  useEffect(() => {
    if (selectedClientId) {
      ...
      if (selectedClient && selectedClient.data.findings.length > 0) {
        const normalizedFindings = normalizeScores(selectedClient.data.findings);
        setFindings(normalizedFindings);
        setHistory(selectedClient.data.history || SEED_HISTORY);
